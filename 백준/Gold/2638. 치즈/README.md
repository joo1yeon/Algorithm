...
- 치즈 안쪽을 ```BFS```로 찾아서 값을 바꾸고 시작하려고 했는데, 중복 코드가 많이 생김
- 치즈 바깥쪽을 찾아서 ```0```, ```1```이 아닌 다른 값(```2```)으로 바꾸고 시작하여 중복을 줄임
- 치즈가 녹았을 때 외부 공기 값(```2```)로 바꾸지 않고 내부 공기 값(```0```)으로 바꾸어 다른 치즈가 녹는 것에 영향을 주지 않았음
