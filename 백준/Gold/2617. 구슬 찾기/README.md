...
- 문제 접근
  - 중간인 구슬이 될 수 없는 구슬 ??? ➡️ 나보다 큰/작은 구슬이 ```N/2+1```개 있다.
  - 구슬의 무게 관계 ➡️ 단방향 그래프를 이용하여 저장
  - 그래프 탐색 방법을 통해 현재 구슬보다 큰/작은 구슬이 몇 개인지 확인
  - 그래프 탐색 방법으로는 플로이드-워셜 알고리즘을 사용
    - N이 크지 않기 때문에 가능
  - row, col 각각을 기준으로 카운트하여 무게 관계를 파악 가능
<br>

 > 참고 자료: [백준 2617 자바 - 구슬 찾기 (BOJ 2617 JAVA) by Nahwasa](https://nahwasa.com/entry/%EB%B0%B1%EC%A4%80-2617-%EC%9E%90%EB%B0%94-%EA%B5%AC%EC%8A%AC-%EC%B0%BE%EA%B8%B0-BOJ-2617-JAVA)
