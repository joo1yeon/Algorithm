...
- Edge Case: 전체 집의 개수와 연속된 집의 개수가 같을 때 (`N == M`)
  - 집마다 보관 중인 금액의 총합이 `M` 미만일 경우 언제나 가능
  - 집마다 보관 중인 금액의 총합이 `M` 이상일 경우 훔치기 불가능
  - 슬라이딩 윈도우 전에 슬라이딩 윈도우 초기값과 `M`을 비교하여 미리 예외 케이스 처리
