...
### 💡 정리
- `sum`을 `long`으로 선언해도 `int`끼리 더하는 과정에서 별도의 타입 캐스팅이 없으면 `int`로 변환되어 저장됨
  - ➡️ 입력 받을 때 부터 `long`으로 받기
- 3개 용액의 합 구하기
  - 투 포인터로 어떻게 3개를 구하지? ➡️ 하나를 고정하고 나머지 2개를 투 포인터로 구함, 그리고 반복  
